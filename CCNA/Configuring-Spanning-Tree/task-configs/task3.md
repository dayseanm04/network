...
SW4 selected its F0/1 interface as the new root port, because it has the lowest root cost of 38 (SW4 f0/1 interface cost: **19** + SW2 F0/3 interface cost **19** + **0** SW1 F0/3 interface cost, equals **28**). A lower root cost is the best path to the root bridge. In case the root cost of SW4 F0/2 interface root cost is 100, which is higher that **19**. **SW4 F0/2 moved to a blocking state**.
